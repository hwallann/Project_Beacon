...
int NBClient::ready()
{
  ...
  switch (_state) {
    ...
    case CLIENT_STATE_CLOSE_SOCKET: {
      // Serial.println("CLIENT_STATE_CLOSE_SOCKET: Closing.");
...
{
  if (_socket != -1) {
    // Serial.println("Stopping in NBClient:connect()");
    stop();
  }
  ...
  if (_synch) {
    while (ready() == 0) {
      if (millis() - start < 150000) {
        // Serial.println("Looping in connect");
        delay(100);
      } else {
        // Serial.println("CONNECT TIMEOUT: " + String(_socket));
        stop();
        return 0;
      }
    }
    ...
    if (_writeSync) {
      ...
      if ( status != 1) {
        if (status == 4 && response.indexOf("Operation not allowed") != -1 ) {
          // Serial.println("NBCLIENT CME ERROR in send");
          stop();
          break;
        } else {
          break;
...
uint8_t NBClient::connected()
{
  ...
  if (NBSocketBuffer.available(_socket) < 0) {
    stop();
...
int NBClient::available()
{
  ...
  if (avail < 0) {
    // Serial.println("NO DATA AVALAIBLE. Stopping");
    stop();
...
void NBClient::stop()
{
  ...
  // Serial.println("Closing in NBClient::stop()");

  // Disable Dormant Close. See 18.24
  MODEM.sendf("AT+USOCLCFG=0");
  MODEM.waitForResponse();

  // Async close: return immediatly
