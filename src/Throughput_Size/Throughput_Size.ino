...
It sends packages of a changing amount of bytes and sends them at fixed times
...
void loop() 
{
  ...
  {
    // Check if an increase in package size is necessary and update if it is
...
char *createMessageToSend(unsigned long size, unsigned long round) {
  ...
  int sLength = 0;
  //Size is the number of bytes in a message package divided by 100. A size of 2 means a 200 B message.
